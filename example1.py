...
class USB_TO_GPIO(object):
    ...
    def __init__(self):
        ...
        try:
            clr.AddReference(
                "C:\\Program Files (x86)\\Texas Instruments\\Fusion Digital Power Studio\\bin\\TIDP.SAA.dll")  # Load the dll file(C:\\ folder)
    ...
    def configure(self, pec_enabled=False):
        ...
        speed = self.device.Set_Bus_Speed(self.device.BusSpeed.Speed100KHz)
    ...
    def i2c_write(self, byteaddr=0x70, bytecmd=0x04, bytedata=None):
        '''
        Wrapper for SAA Write Byte Function, explicitly states which overload to use in Fusion API
        Args:
            (uint8) dev_addr = PMBus Slave Address (0-127) - b'\x7F'
